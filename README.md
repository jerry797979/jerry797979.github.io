...
### 👉 서버에 설치하실 분은 **[설치안내.md](설치안내.md)** 를 보세요

Ubuntu + nginx 에 올려 `https://ziotes.com` 으로 서비스하는 전체 절차를
따라 하기만 하면 되도록 정리해 두었습니다. nginx 설정 파일 전문, HTTPS 발급,
상담폼 설정, 설치 후 확인 목록까지 들어 있습니다.

---

`dist/` 폴더가 웹 루트입니다. 그대로 올리면 됩니다.

| 올리는 곳 | 방법 | PHP 기능 |
|---|---|---|
| **Ubuntu + nginx** | [설치안내.md](설치안내.md) 참고 | ✅ 전부 동작 |
| **Cloudflare Pages** | 빌드 명령 없음, 출력 디렉터리 `dist` | ❌ (Functions로 다시 작성 필요) |
| **GitHub Pages** | Settings → Pages → Source 를 `GitHub Actions` 로 | ❌ |
...
> (지역 페이지 53,238장)는 **PHP가 도는 서버에서만 동작**합니다.
> GitHub Pages·Cloudflare Pages 에서는 상담 신청이 저장되지 않고
> 지역 페이지도 뜨지 않습니다.

> ⚠️ **nginx 를 쓰신다면** 저장소의 `.htaccess` 는 **무시됩니다.**
> 상담 신청 원본(`_leads/`)과 지역 데이터(`_data/`)를 막는 설정을
> 직접 넣지 않으면 주소만 알면 열립니다. [설치안내.md](설치안내.md) 의
> nginx 설정을 그대로 쓰세요.
